...
if __name__ == "__main__":
    ...
    def test_fake_stream_request():
        """测试fake-stream请求"""
        print("\n" + "=" * 80)
        print("【测试4】fake-stream请求 (POST /v1/models/fake-stream/gemini-2.5-flash:streamGenerateContent)")
        print("=" * 80)
        print(f"请求体: {json.dumps(test_request_body, indent=2, ensure_ascii=False)}\n")
        ...
        with client.stream(
            "POST",
            "/v1/models/fake-stream/gemini-2.5-flash:streamGenerateContent",
            json=test_request_body,
            params={"key": test_api_key}
        ) as response:
            print(f"状态码: {response.status_code}")
    ...
    def test_anti_truncation_stream_request():
        """测试anti-truncate请求"""
        print("\n" + "=" * 80)
        print("【测试5】anti-truncate请求 (POST /v1/models/anti-truncate/gemini-2.5-flash:streamGenerateContent)")
        print("=" * 80)
        print(f"请求体: {json.dumps(test_request_body, indent=2, ensure_ascii=False)}\n")
        ...
        with client.stream(
            "POST",
            "/v1/models/anti-truncate/gemini-2.5-flash:streamGenerateContent",
            json=test_request_body,
            params={"key": test_api_key}
        ) as response:
            print(f"状态码: {response.status_code}")
